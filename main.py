...
def existing_user_valid(USERS: dict) -> str:
  while True:
    if USERS == {}:
      print(
        colored("\nThere are currently no users saved.",
                "yellow",
                attrs=["bold"]))
      return "quit"
...
def append_csv(path) -> None:
  ...
  input_purchase = input(
    "Enter the location of the purchase > ").title().strip()
...
def append_past_csv(path, action):
  if os.path.exists(f"{path}/{action}.csv"):
    with open(f"{path}/{action}.csv", "a") as current_file:
      ...
      csv_writer.writerow(row)
    print(
      colored(f"\n<<<< The report for {action} has been updated! >>>>",
              attrs=["bold"]))
    time.sleep(2)
...
def write_csv(path) -> None:
  ...
  if not os.path.exists(f"{path}/{month_time_str}.csv"):
    with open(f"{path}/{month_time_str}.csv", "w+",
              newline="") as current_file:
